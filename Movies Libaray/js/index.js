...
fetchDataFromServer(`https://api.themoviedb.org/3/genre/movie/list?api_key=${api_key}`, function ({ genres }) {
    ...
    for (const { title, path } of homePageSections) {
        fetchDataFromServer(`https://api.themoviedb.org/3/${path}?api_key=${api_key}`, createMediaList, title);
    }
});
    ...
    for (const [index, movie] of movieList.entries()) {
        ...
        sliderItem.innerHTML = `
            ...
            <div class="banner-content">
                ...
                <i class="fa-solid fa-play" id="play" aria-label="Watch Now"></i>
...
const createMediaList = function ({ results: mediaList }, title) {
    const mediaListElem = document.createElement("section");
    mediaListElem.classList.add("movie-list");
    mediaListElem.ariaLabel = `${title}`;
    ...
    for (const media of mediaList) {
        const mediaCard = title.includes("TV Shows") ? createTVShowCard(media) : createMovieCard(media);
        mediaListElem.querySelector(".slider-inner").appendChild(mediaCard);
    }

    pageContent.appendChild(mediaListElem);
};
